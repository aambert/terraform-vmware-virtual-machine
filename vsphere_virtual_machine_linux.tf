resource "vsphere_virtual_machine" "linux" {
  count = "${var.virtual_machine_template_os_family == "linux" ? var.virtual_machine_instance_count : 0}"

  name             = "${format("%s-%d", var.virtual_machine_name_prefix, count.index)}"
  resource_pool_id = "${var.resource_pool != "" ? data.vsphere_resource_pool.resource_pool.id : data.vsphere_compute_cluster.cluster.resource_pool.id}"
